    ...
    if source.is_file() {
        ...
        if let Some(true) = recursive {
            ...
            for entry in fs::read_dir(source)? {
                ...
                move_item(&entry_path, &dest_path, recursive, force)?;
            }
            ...
            return Err(io::Error::new(
                io::ErrorKind::InvalidInput,
                "Cannot move directory without resursive parameter.",
            ));
        }
    }
    ...
    if let Some(params) = parameters {
        ...
        if let Some(source_str) = params.get("source").and_then(Value::as_str) {
            if let Some(destination_str) = params.get("destination").and_then(Value::as_str) {
                ...
                match move_item(source, destination, Some(recursive), Some(force)) {
                    Ok(()) => {
